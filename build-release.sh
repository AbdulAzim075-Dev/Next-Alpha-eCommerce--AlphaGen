#!/usr/bin/env bash
#
# build-release.sh — package Next Alpha Ecommerce into a release-ready zip.
#
...
if [[ ! -f "$STAGE/vendor/autoload.php" ]]; then
    echo "ERROR: stage has no vendor/autoload.php — a release package must run without composer." >&2
    exit 1
fi
